...
import { WorkMusicalMetadata, NotablePerformance } from './metadata/types';

/**
 * Musical metadata for specific works
 * Work is done in batches - see batch tracking at the bottom
 * 
 * NOTE: Individual work metadata has been moved to separate files in the /metadata directory
 ...
 * Now uses the aggregated metadata from separate files
 */
export function getWorkMusicalMetadata(identifier: string): WorkMusicalMetadata | null {
  // Import the aggregated metadata
  const { getWorkMusicalMetadata: getMetadata } = require('./metadata');
...
export function hasCompleteMetadata(identifier: string): boolean {
  const { hasCompleteMetadata: hasComplete } = require('./metadata');
  return hasComplete(identifier);
}
...
export function getCompletedWorks(): string[] {
  const { getCompletedWorks: getCompleted } = require('./metadata');
  return getCompleted();
}
...
export function getCompletionStats() {
  const { getCompletionStats: getStats } = require('./metadata');
