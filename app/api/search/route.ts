...
// Create a cached search function with proper cache key generation
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use Next.js caching with proper cache key including all parameters
    const cacheKey = `search-${query}-${creator}-${date}-${language}-${page}-${rows}`;
    const result = await unstable_cache(
      () => getCachedSearchResults(query, creator, date, language, page, rows),
      [cacheKey],
      {
